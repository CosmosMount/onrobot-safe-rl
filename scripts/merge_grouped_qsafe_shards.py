...
def _reject_locked_audit_basenames(
    ...
    """Keep the generic merger from touching any locked audit component.
    ...
    for path in paths:
        try:
            assert_no_locked_audit_path_components(path)
        except ProtectedEvidencePathError as exc:
            raise ValueError(
                "locked audit paths are forbidden in the generic merger") from exc


def _reject_generic_reserved_workflow_paths(
    paths: Sequence[str | os.PathLike[str]],
) -> None:
    """Reject any workflow-owned generic path without probing it."""
    for path in paths:
        try:
            assert_generic_evidence_path(path)
        except ProtectedEvidencePathError as exc:
            raise ValueError(
                "reserved workflow paths are forbidden in the generic merger") \
                from exc
...
def _reject_locked_v3_audit_basenames(
    ...
    """Backward-compatible alias for the V3+V4+V5 lexical firewall."""
...
def main() -> int:
    ...
    # A caller may try to smuggle a workflow artifact through --protocol.
    # Reject that spelling before the protocol guard can stat or read it.
    _reject_generic_reserved_workflow_paths([args.protocol])
    ...
    if is_v3:
        ...
        # This is still before the first existence check or load of any
        # artifact argument.  Canonical V3 is the sole generic-workflow
        # exception and already passed its exact discovery allowlist above.
        _reject_generic_reserved_workflow_paths(supplied_artifacts)
        if args.collection_reports:
    ...
    for path in args.shards:
        if is_v3:
            with workflow_evidence_read_scope(
                    ...
                    role="discovery",
                    path=path):
                datasets.append(GroupedBranchDataset.load(path))
        else:
            datasets.append(GroupedBranchDataset.load(path))
        ...
        for path, dataset in zip(
            ...
            if is_v3:
                with workflow_evidence_read_scope(
                        ...
                        path=path):
                    views.append(PrivilegedBranchView.load(
