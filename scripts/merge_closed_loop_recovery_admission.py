...
def main() -> int:
    ...
    ledgers = [AdmissionLedger.load(path) for path in args.admission_shards]
    views = [
        AdmissionPrivilegedView.load(path, ledger=ledger)
        for path, ledger in zip(
            args.privileged_shards, ledgers, strict=True)
    ]
