#!/usr/bin/env python3
"""Legacy fallback A/B for Stage-2 shielding (withdrawn).
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--output-dir',
        default='saved/safety_evaluation/fallback_ab_ref15327')
    ...
    try:
        for attempt in range(attempts):
            try:
                ...
                print(f'[ab] stabilized attempt={attempt}', flush=True)
                time.sleep(1.0)
                return
            except UnstableResetError:
                time.sleep(2.0)
        raise RuntimeError('Could not stabilize before fallback A/B cell')
    finally:
        env.close()
