...
def _lexical_safe_path(path: str | Path) -> Path:
    ...
    for component in lexical.parts[:-1]:
        if component in _LOCKED_V3_AUDIT_BASENAMES:
            raise ProtectedEvidencePathError(
...
def assert_safe_evidence_output(path: str | Path) -> Path:
    ...
    if lexical.name in _RESERVED_V3_OUTPUT_BASENAMES:
        raise ProtectedEvidencePathError(
...
def require_v3_audit_consumed_or_safe_input(path: str | Path) -> Path:
    ...
    All non-audit evidence inputs must be regular final components rather than
    symlinks.  A canonical v3 audit basename additionally requires the exact
    ...
    if lexical.name not in _LOCKED_V3_AUDIT_BASENAMES:
        try:
            metadata = lexical.lstat()
        except OSError:
            return lexical
    ...
    if not isinstance(contract, Mapping) or set(contract) != marker_fields or (
            ...
            "qsafe.closed_loop_recovery_triage.audit_consumed.v1") or (
                contract.get("protocol_name") !=
                "objective1_closed_loop_recovery_triage_v3") or (
                    contract.get("status") !=
        ...
        raise ProtectedEvidencePathError(
            "v3 audit-consumed marker contract is invalid")
    ...
    if not isinstance(lock, Mapping) or hashlib.sha256(lock_bytes).hexdigest() != (
        ...
        raise ProtectedEvidencePathError(
            "v3 audit-consumed marker is not bound to the selection lock")
