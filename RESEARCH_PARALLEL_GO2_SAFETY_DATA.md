...
**Post-report implementation audit (2026-08-09):** the initial check measured about 0.0196 maximum observation error after restore. Follow-up isolated this to comparing `mj_step`'s stale derived sensor fields with a post-`mj_forward` restore; qpos/qvel/integration state and duplicate action branches were exact. Explicit forward synchronization reduces raw observation error to zero. The more important defects were real: legacy P17 supplied a normalized previous action where corrected 46D requires absolute `q_send`, kept four rather than five frames, and omitted action-filter history. Phase 1A now uses sensor/frame-correct reads plus a compound snapshot containing requested/executed/q-target actions, five observations, and filter history. Legacy P17 artifacts remain diagnostic-only because fixing the collector cannot retroactively repair already generated files.
